...
class SimplePager {
    ...
    public function html($href = '', $attr = '') {
        ...
        echo "<a href='?page=$prev&$href'>Back</a>";
    
        // Dropdown list for pagination
        echo "<select id='page-dropdown' onchange='navigateToPage(this.value)'>";
        for ($p = 1; $p <= $this->page_count; $p++) {
            $selected = $p == $this->page ? 'selected' : '';
            echo "<option value='$p' $selected>Page $p</option>";
        }
        echo "</select>";
        ...
        <script>
            function navigateToPage(page) {
                window.location.href = '?page=' + page + '&$href';
            }
        </script>";
    }
